...
(function() {
  ...
  const NotLettersAPI = {
    ...
    extractVerificationCode(messageContent) {
      ...
      console.log('📧 Длина письма:', messageContent.length, 'символов');
      
      // Более точные паттерны (по приоритету)
      const patterns = [
        // Специфичные паттерны для Cursor (высокий приоритет)
        /(?:код|code)[^\d]*(\d{6})/i,                    // "код 123456" или "code 123456"
        /verification[^\d]*(\d{6})/i,                    // "verification 123456"
        /confirm[^\d]*(\d{6})/i,                         // "confirm 123456"
        /введите[^\d]*(\d{6})/i,                         // "введите 123456"
        /enter[^\d]*(\d{6})/i,                           // "enter 123456"
        
        // Поиск изолированного 6-значного числа в HTML тегах
        />\s*(\d{6})\s*</,                               // >123456<
        /<p[^>]*>\s*(\d{6})\s*<\/p>/,                   // <p>123456</p>
        /<div[^>]*>\s*(\d{6})\s*<\/div>/,               // <div>123456</div>
        /<h\d[^>]*>\s*(\d{6})\s*<\/h\d>/,               // <h1>123456</h1>
      ...
      for (const pattern of patterns) {
        ...
        if (match && match[1]) {
          ...
          if (uniqueDigits === 1) {
            console.log(`⚠️ Найден подозрительный код ${code} (все цифры одинаковые), пропускаем...`);
            continue;
          }
          console.log('✓ Код найден:', code, `(паттерн: ${pattern})`);
          return code;
        }
      }
      
      // Если ничего не найдено специфичными паттернами, ищем любое 6-значное число
      // НО исключаем повторяющиеся цифры
      const allSixDigits = messageContent.match(/\b\d{6}\b/g);
      if (allSixDigits && allSixDigits.length > 0) {
        console.log('🔍 Найдено 6-значных чисел:', allSixDigits);
        for (const code of allSixDigits) {
          const uniqueDigits = new Set(code.split('')).size;
          if (uniqueDigits > 1) { // Не все цифры одинаковые
