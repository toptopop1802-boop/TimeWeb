...
namespace Oxide.Plugins
{
    ...
    public class RustApp : RustPlugin
    {
        ...
        private static class CourtApi
        {
            ...
            {
                // Можно использовать переменную окружения или конфигурацию
                // Для примера используем относительный путь, который будет работать с тем же доменом
                // Если нужен абсолютный URL, раскомментируйте и укажите ваш домен:
                // return "https://your-domain.com";
                return ""; // Относительный путь - будет использовать тот же домен, что и плагин
            }
            ...
            {
                // Используем наш API вместо court.rustapp.io
                var customApiUrl = GetCustomApiBaseUrl();
                var endpoint = string.IsNullOrEmpty(customApiUrl) 
                    ? "/api/rust/kills/report"  // Относительный путь
                    : $"{customApiUrl}/api/rust/kills/report";  // Абсолютный путь
                
                return new StableRequest<object>(endpoint, RequestMethod.POST, payload);
            }
        ...
        private class PlayerMuteWorker : RustAppWorker
        {
            ...
            public void RemovePlayerMuteBySteamId(string steamId)
            {
                if (!ulong.TryParse(steamId, out var userId))
                {
                    return;
                ...
                if (PlayerMutes.ContainsKey(userId))
                {
                    PlayerMutes.Remove(userId);
                }
            }

            public void AddLocalMute(string steamId, string reason, string duration)
            {
                if (!ulong.TryParse(steamId, out var userId))
                {
                    return;
                ...
                var timeMs = ParseDurationToMilliseconds(duration);
                if (timeMs <= 0)
                {
                    return;
                }

                var muteDto = new CourtApi.PlayerMuteDto
                {
                    target_steam_id = steamId,
                    reason = reason,
                    left_time_ms = timeMs,
                    received_at = DateTimeOffset.Now.ToUnixTimeMilliseconds()
                };

                AddPlayerMute(muteDto);
            }

            private long ParseDurationToMilliseconds(string duration)
            {
                if (string.IsNullOrEmpty(duration))
                    return 0;

                duration = duration.Replace(" ", "").ToLower();
                long totalMs = 0;
        ...
        private void CmdConsoleMute(ConsoleSystem.Arg args)
        {
            ...
            if (player != null && !player.IsAdmin && !permission.UserHasGroup(player.UserIDString, "owner"))
            {
                return;
            }

            var allArgs = args.Args ?? Array.Empty<string>();
            var clearArgs = allArgs.Where(v => v != "--broadcast" && v != "--no-broadcast").ToList();
            
            if (clearArgs.Count < 3)
            {
                Error("Incorrect command format!\nCorrect format: ra.mute <steam-id> <reason> <time>\n\nAdditional options are available:\n'--no-broadcast' - disable broadcast notification\n\nExample: ra.mute 7656119812110397 \"Спам в чате\" 1h");
                return;
            }

            // Первый аргумент - steam-id, последний - время, все между ними - причина
            var steamId = clearArgs[0];
            var duration = clearArgs[clearArgs.Count - 1];
            var reason = string.Join(" ", clearArgs.Skip(1).Take(clearArgs.Count - 2));

            // По умолчанию broadcast включен, можно отключить через --no-broadcast
            var broadcast_bool = !args.FullString.Contains("--no-broadcast");
        ...
        private void RustApp_PlayerMuteCreate(string targetSteamId, string reason, string duration, string comment = null, string referenceMessageText = null, bool broadcast = false)
        {
            ...
            request.Execute(
                ...
                (err) =>
                {
                    // Если ошибка связана с тарифом, создаем мут локально
                    if (err.Contains("тарифа") || err.Contains("tariff") || err.Contains("bad request") || err.Contains("400"))
                    {
                        _RustAppEngine?.PlayerMuteWorker?.AddLocalMute(targetSteamId, reason, duration);
                        Puts($"Player ({targetSteamId}) is muted locally (tariff limitation bypassed)");
                        
                        // Если нужен broadcast, отправляем сообщение всем игрокам
                        if (broadcast)
                        {
                            var target = BasePlayer.Find(targetSteamId);
                            var mute = _RustAppEngine?.PlayerMuteWorker?.GetMute(ulong.Parse(targetSteamId));
                            if (mute != null && target != null)
                            {
                                foreach (var player in BasePlayer.activePlayerList)
                                {
                                    var msg = lang.GetMessage("System.Mute.Broadcast.Mute", this, player.UserIDString)
        ...
        private void RustApp_PlayerMuteDelete(string targetSteamId)
        {
            ...
            request.Execute(
                (obj) =>
                {
                    ...
                    // Удаляем локально на всякий случай
                    _RustAppEngine?.PlayerMuteWorker?.RemovePlayerMuteBySteamId(targetSteamId);
                },
                (err) =>
                {
                    // Если ошибка связана с тарифом, удаляем мут локально
                    if (err.Contains("тарифа") || err.Contains("tariff") || err.Contains("bad request") || err.Contains("400"))
                    {
                        _RustAppEngine?.PlayerMuteWorker?.RemovePlayerMuteBySteamId(targetSteamId);
                        Puts($"Player ({targetSteamId}) is unmuted locally (tariff limitation bypassed)");
                    }
                    else
                    {
