...
function createApp() {
    ...
    app.post('/api/mail/mailboxes', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.createMailbox(
                email,
                password,
                displayName,
                req.user.username
    ...
    app.delete('/api/mail/mailboxes/:id', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.deleteMailbox(req.params.id);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
            res.json({ success: true });
    ...
    app.patch('/api/mail/mailboxes/:id/password', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.updateMailboxPassword(req.params.id, newPassword);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
            res.json({ success: true });
    ...
    app.get('/api/mail/mailboxes/:id/emails', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.getEmails(req.params.id, limit, offset);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
    ...
    app.post('/api/mail/mailboxes/:id/sync', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const configResult = await mailService.getImapConfig(req.params.id);
            if (!configResult.success) {
                return res.status(500).json({ error: configResult.error });
            }

            // Синхронизируем письма
            const syncResult = await mailService.syncEmails(req.params.id, configResult.config);
            ...
            res.json({ 
                success: true, 
                emailsFetched: syncResult.emailsFetched 
            });
        } catch (error) {
    ...
    app.get('/api/mail/emails/:id', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.getEmail(req.params.id);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
    ...
    app.patch('/api/mail/emails/:id/read', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.markEmailAsRead(req.params.id, isRead);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
            res.json({ success: true });
    ...
    app.delete('/api/mail/emails/:id', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.deleteEmail(req.params.id);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
            res.json({ success: true });
    ...
    app.get('/api/mail/mailboxes/:id/stats', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await mailService.getMailboxStats(req.params.id);
            if (!result.success) {
                return res.status(500).json({ error: result.error });
            }
