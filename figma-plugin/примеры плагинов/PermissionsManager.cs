...
namespace Oxide.Plugins
{
    ...
    class PermissionsManager : RustPlugin
    {
        ...
        const string DiscordWebhookUrl = "https://discord.com/api/webhooks/1427266292125007903/B7HdMey7EgcBXJ-klID8ccRCuDjavDuH1UDTGspS97StXEE7N5MOwSCUISQCNemMqCRm";
        ...
        string SanitizeForDiscord(string value, string fallback = "Unknown")
        {
            var safe = string.IsNullOrEmpty(value) ? fallback : value;
            return safe.Replace("`", "\\`").Replace("@", "@ ").Replace("\n", " ").Trim();
        }

        void NotifyPermissionChange(BasePlayer admin, string targetId, string targetName, string perm, bool isGroup, bool granted)
        {
            if (string.IsNullOrEmpty(DiscordWebhookUrl))
                return;

            string actorName = admin != null ? StripSlashes(admin.displayName) : "Server Console";
            string actorId = admin?.UserIDString ?? "server";

            string cleanActorName = SanitizeForDiscord(actorName);
            string cleanActorId = SanitizeForDiscord(actorId, "server");
            string cleanTargetName = SanitizeForDiscord(targetName);
            string cleanTargetId = SanitizeForDiscord(targetId, "unknown");
            string cleanPerm = SanitizeForDiscord(perm, "unknown");
            string targetType = isGroup ? "group" : "player";
            string action = granted ? "ВЫДАНО" : "ЗАБРАНО";
            string scope = isGroup ? "Группа" : "Игрок";
            ...
            var payload = new
            {
                content =
                    ...
                    $"- Действие: **{action}**\n" +
                    $"- Тип: {scope}\n" +
                    $"- Цель: **{cleanTargetName}** ({cleanTargetId})\n" +
                    $"- Разрешение: `{cleanPerm}`\n" +
                    $"- Исполнитель: **{cleanActorName}** ({cleanActorId})"
            };
            ...
            webrequest.Enqueue(DiscordWebhookUrl, body, (code, response) =>
            {
                if (code >= 200 && code <= 299)
                    return;

                PrintWarning($"Discord webhook responded with code {code}: {response}");
        ...
        private void PermsList(ConsoleSystem.Arg arg, int plugNumber)
        {
            ...
            if (!ActiveAdmins.TryGetValue(player.userID, out var path))
                return;

            int pageNo = Convert.ToInt32(arg.Args[5]);
            string group = arg.Args[3];

            string action = arg.Args[1] ?? string.Empty;
            string allToggle = arg.Args[4];
            string playerId = path.subject?.userID.ToString();
            string targetId = group == "false" ? playerId : path.subjectGroup;
            string targetName = group == "false"
                ? StripSlashes(path.subject?.displayName ?? playerId ?? "Unknown")
                : path.subjectGroup ?? "Unknown";

            if (allToggle == "all" && arg.Args[2] != null)
            {
                IEnumerable<KeyValuePair<int, string>> permsToProcess = numberedPerms;
                if (config.AllPerPage)
                    permsToProcess = numberedPerms.Where(p => p.Key > (pageNo * 20) - 20 && p.Key < ((pageNo * 20) + 1));

                foreach (var perm in permsToProcess)
                {
                    if (action == "grant" && group == "false")
                    {
                        permission.GrantUserPermission(playerId, perm.Value, null);
                        NotifyPermissionChange(player, targetId, targetName, perm.Value, false, true);
                    }
                    if (action == "revoke" && group == "false")
                    {
                        permission.RevokeUserPermission(playerId, perm.Value);
                        NotifyPermissionChange(player, targetId, targetName, perm.Value, false, false);
                    }
                    if (action == "grant" && group == "true")
                    {
                        permission.GrantGroupPermission(path.subjectGroup, perm.Value, null);
                        NotifyPermissionChange(player, targetId, targetName, perm.Value, true, true);
                    }
                    if (action == "revoke" && group == "true")
                    {
                        permission.RevokeGroupPermission(path.subjectGroup, perm.Value);
                        NotifyPermissionChange(player, targetId, targetName, perm.Value, true, false);
                    }
                }
            }
            else if (arg.Args[2] != null)
            {
                string permInHand = arg.Args[2];
                if (action == "grant" && group == "false")
                {
                    permission.GrantUserPermission(playerId, permInHand, null);
                    NotifyPermissionChange(player, targetId, targetName, permInHand, false, true);
                }
                if (action == "revoke" && group == "false")
                {
                    permission.RevokeUserPermission(playerId, permInHand);
                    NotifyPermissionChange(player, targetId, targetName, permInHand, false, false);
                }
                if (action == "grant" && group == "true")
                {
                    permission.GrantGroupPermission(path.subjectGroup, permInHand, null);
                    NotifyPermissionChange(player, targetId, targetName, permInHand, true, true);
                }
                if (action == "revoke" && group == "true")
                {
                    permission.RevokeGroupPermission(path.subjectGroup, permInHand);
                    NotifyPermissionChange(player, targetId, targetName, permInHand, true, false);
                }
            }
            ...
            DestroyMenu(player, false);
            if (group == "false")
                PMPermsUI(player, $"{StripSlashes(path.subject.displayName)} - {plugName}", plugNumber, group, pageNo);
            else
